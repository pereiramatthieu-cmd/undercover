...
export default function DessinResults({ dessinState, myId, onGoHome }) {
  if (!dessinState) return null;

  const { players = [], category, totalRounds } = dessinState;
  ...
  const CATS = { aleatoire: "Aléatoire", shonen: "Shōnen", shojo: "Shōjo", seinen: "Seinen" };
      ...
      <div className="page-inner gap-24 fade-in">
        ...
        <div className="text-center gap-8">
          <span className="badge badge-accent">
            {totalRounds} manches · {CATS[category] || category}
          </span>
        ...
        {winner && winner.score > 0 && (
          <div style={{
            ...
            <div style={{ fontFamily: "Syne, sans-serif", fontSize: "3rem", fontWeight: 800, marginBottom: 8 }}>🏆</div>
            ...
            {sorted.map((p, i) => {
              ...
              return (
                <div key={p.id} style={{
                  ...
                  border: `1px solid ${i === 0 ? "rgba(124,106,247,0.35)" : "var(--border)"}`,
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      ...
                      <span style={{ fontWeight: 700 }}>
                        {p.name}
                        {isMe && <span style={{ fontSize: "0.75rem", color: "var(--text-muted)", fontWeight: 400, marginLeft: 6 }}>(toi)</span>}
                      </span>
                    </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {isHost ? (
            <button className="btn btn-primary btn-full" onClick={() => socket.emit("dessin:restart", {})}>
              Rejouer →
            </button>
          ) : (
